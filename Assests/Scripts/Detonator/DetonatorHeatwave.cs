...
public class DetonatorHeatwave : DetonatorComponent {
	...
	private float _baseDuration = .125f;
	...
	void Update () 
	{
		...
		if (_heatwave)
		{
			...
			Vector3 tmp = GlobalInfo.playerCamera.transform.position - _heatwave.gameObject.transform.position;
			tmp.Normalize();
			Vector3 tmp1 = Vector3.Cross(tmp,Vector3.up);
			tmp1 = _heatwave.gameObject.transform.position + tmp1;
			_heatwave.gameObject.transform.LookAt(tmp1,tmp);
